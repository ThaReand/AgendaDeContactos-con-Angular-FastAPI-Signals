...
export class ContactosService {
  ...
  private categoriaFiltroSignal = signal<string | null>(null);
  categoriaFiltro = this.categoriaFiltroSignal.asReadonly();
  /**
   * ----------------------------------------------------------
   * Signal de carga
   * ----------------------------------------------------------
  ...
  contactosFiltrados = computed(() => {
    ...
    const categoria = this.categoriaFiltroSignal();
    let lista = this.contactosSignal();

    // Primer paso: Filtrar por categoría (si hay una seleccionada)
    if (categoria) {
      lista = lista.filter(contacto =>
        contacto.categoria.toLowerCase() === categoria.toLowerCase()
      );
    }

    // Segundo paso: Filtrar por texto acumulado en el buscador
    if (!texto) {
      return lista;
    }

    return lista.filter(contacto =>
  ...
  actualizarFiltroCategoria(categoria: string | null): void {
    this.categoriaFiltroSignal.set(categoria);
  }
  /**
   * ----------------------------------------------------------
   * agregar()
